...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            if rank == -1:
                ...
                if gpu_ids:
                    ...
                    try:
                        ...
                        print(f"[DEBUG] Inferred Rank: {rank}, World Size: {world_size} from Ray GPU IDs: {gpu_ids}",
                              flush=True)
                    except ValueError:
            ...
            print(f"[DEBUG] Final Configuration - Rank: {rank}, Local Rank: {local_rank}, World Size: {world_size}",
                  flush=True)
            print(f"[DEBUG] Master Addr: {os.environ.get('MASTER_ADDR')}, Master Port: {os.environ.get('MASTER_PORT')}",
                  flush=True)
...
class CriticWorker(Worker):
    ...
    def init_model(self):
        ...
        self.critic = DataParallelPPOCritic(config=self.config, critic_module=self.critic_module,
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                output = self.reward_module(input_ids=input_ids_rmpad, attention_mask=None,
                ...
                if self.ulysses_sequence_parallel_size > 1:
                    reward_rmpad = gather_outpus_and_unpad(reward_rmpad, gather_dim=0, unpad_dim=0,
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            prompt_with_chat_template = target_tokenizer.apply_chat_template(chat, add_generation_prompt=False,
                                                                             tokenize=False)
